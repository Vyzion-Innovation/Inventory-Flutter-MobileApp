 ...
 @override
  ...
  Future<void> fetchBuyers() async {
    try {
      ...
      buyers = snapshot.docs.map((doc) => CustomerModel.fromFirestore(doc)).toList();
    } catch (e) {
  ...
  Future<void> fetchSuplliers() async {
    try {
      ...
      sellers = querySnapshot.docs
          .map((doc) => SupplierModel.fromFirestore(doc)).toList();
  ...
  void setInventoryData(InventoryModel inventory) {
    ...
    // buyerNameController.text = inventory.buyer.toString();
  ...
  try {
    ...
    await FirebaseFirestore.instance.collection('inventories').add(inventoryData);
...
Future<void> _updateIntemInventory() async {
    ...
    if (purchaseDateController.text.isNotEmpty) {
     ...
     if (sellDateController.text.isNotEmpty) {
      ...
      if (inventoryToEdit == null) return; // Ensure there's a customer to update
